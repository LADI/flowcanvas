 ...
 *
 * Eugene is free software; you can redistribute it and/or modify it under the
 * terms of the GNU General Public License as published by the Free Software
 * Foundation; either version 2 of the License, or (at your option) any later
 * version.
 *
 * Eugene is distributed in the hope that it will be useful, but WITHOUT ANY
...
#ifndef EUGENE_GLADEFILE_HPP
#define EUGENE_GLADEFILE_HPP
...
public:
	static Glib::RefPtr<Gnome::Glade::Xml> open(const std::string& base_name) {
		...
		if (fs.fail()) { // didn't find it, check DATA_PATH
			fs.clear();
			glade_filename = std::string(EUGENE_DATA_DIR).append("/").append(base_name).append(".glade");

			fs.open(glade_filename.c_str());
			if (fs.fail()) {
				std::ostringstream ss;
				ss << "Unable to find " << base_name << "glade in current directory or "
					<< EUGENE_DATA_DIR << std::endl;
