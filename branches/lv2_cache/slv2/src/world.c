...
{
	const char* xdg_cache_home = NULL;
	const char* home           = NULL;
	const char* prefix         = uri ? "file://" : "";
	char*       cache_path     = NULL;
	if ((xdg_cache_home = getenv("XDG_CACHE_HOME"))) {
		cache_path = slv2_strjoin(prefix, xdg_cache_home, "/slv2/", name, NULL);
	} else if ((home = getenv("HOME"))) {
		cache_path = slv2_strjoin(prefix, home, "/.cache/slv2/", name, NULL);
	} else {
		cache_path = slv2_strjoin(prefix, "/var/cache/slv2/", name, NULL);
	}
	return cache_path;
}
...
static void
slv2_world_load_cache(SLV2World world)
{
	// Read world file timestamps
	char* files_uri_str = slv2_world_cache_location("files.nt", true);
	librdf_uri* files_uri = librdf_new_uri(world->world, (const unsigned char*)files_uri_str);
	librdf_parser* parser = librdf_new_parser(world->world, "ntriples", NULL, NULL);
	librdf_parser_parse_into_model(parser, files_uri, NULL, world->files);
	librdf_free_parser(parser);
	librdf_free_uri(files_uri);
	free(files_uri_str);
}
...
slv2_world_new_internal(SLV2World world)
{
	...
	// Create cache directory (e.g. ~/.cache/slv2) if necessary
	char* dir_path = slv2_world_cache_location("", false);
	struct stat dir;
	stat(dir_path, &dir);
	...
	world->files_storage = slv2_world_new_memory_storage(world, false);
	if (!world->files_storage)
		goto fail;

	world->files = librdf_new_model(world->world, world->files_storage, NULL);
	if (!world->files)
		goto fail;
	...
	slv2_world_load_cache(world);
	return world;

fail:
	librdf_free_storage(world->storage);
	librdf_free_model(world->model);
	librdf_free_model(world->files);
	librdf_free_storage(world->files_storage);
...
slv2_world_new_memory_storage(SLV2World world, bool index)
{
	...
	librdf_storage* ret = librdf_new_storage(world->world, "trees", NULL,
			index ? "index-spo='yes',index-ops='yes'" : "index-spo='yes'");
	if (!ret) {
		warned = true;
...
static void
slv2_world_save_cache(SLV2World world)
{
	char* dir_path = slv2_world_cache_location("", false);
	...
	// Write world file timestamps
	librdf_serializer* serializer = librdf_new_serializer(world->world, "ntriples", NULL, NULL);
	char* files_path = slv2_world_cache_location("files.nt", false);
	librdf_serializer_serialize_model_to_file(serializer, files_path, NULL, world->files);
	free(files_path);
...
slv2_world_load_bundle(SLV2World world, SLV2Value bundle_uri)
{
	...
	for (; !librdf_query_results_finished(query_results); librdf_query_results_next(query_results)) {
		...
		librdf_storage* dyn_manifest_storage = slv2_world_new_memory_storage(world, true);
